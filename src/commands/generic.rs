	...
	($cname:ident, $action:expr, $desc:expr, $usage:expr, $example:expr, $extra:expr) => {
		...
		async fn $cname(ctx: &Context, msg: &Message, mut args: Args) -> CommandResult {
		    ...
		    msg.channel_id
		        .say(
		            &ctx,
		            format!("***{}** {} **{}**{}*", author.display_name(), $action, member.display_name(), $extra),
		        )
		        .await?;
...
text_command!(headpat, "headpats", "Headpats another user.", "headpat <user>", "headpat Elinvynia", "");
text_command!(bonk, "bonks", "Bonks another user to horny jail.", "bonk <user>", "bonk Elinvynia", ". Go to horny jail!");
